...
export const LoginScreen: React.FC<LoginScreenProps> = ({
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Validate account if found in database
      if (matchedAccount) {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    const selectedOrg = orgSuggestions.find((o) => o.id.toUpperCase() === cleanOrgId);
    const orgName = selectedOrg ? selectedOrg.name : companySettings?.orgName || `Đơn vị ${cleanOrgId}`;
          ...
          {authMode === 'register' && (
            <div>
              <div className="mb-4 pb-2 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-800">Đăng ký tài khoản nhân sự mới</h3>
                  <p className="text-[11px] text-slate-400">Tạo tài khoản và đồng bộ lên Realtime Database</p>
                </div>
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { id: 'admin', label: 'Quản Trị', desc: 'Toàn quyền hệ thống' },
                    ].map((roleItem) => (
                      <button
                        key={roleItem.id}
                        type="button"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-slate-700">Mã Doanh Nghiệp</label>
                    <input
                      type="text"
                      value={regOrgId}
