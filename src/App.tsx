  ...
  return (
    <div className="min-h-screen bg-[#f8fafd] text-slate-800 flex">
      ...
      <Sidebar
        ...
        onOpenTenantManager={() => setIsTenantManagerOpen(true)}
      />
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-blue-50 border border-slate-200/90 hover:border-blue-300 text-xs font-semibold text-slate-800 hover:text-blue-700 transition-all cursor-pointer shadow-2xs group"
              title="Nhấn để đổi không gian chi nhánh hoặc quản lý Multi-Tenant"
            >
              <Building2 className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" />
              <div className="text-left hidden sm:block">
                <span className="text-[10px] text-slate-400 block font-normal leading-none">Chi nhánh</span>
                <span className="font-bold text-slate-900 group-hover:text-blue-700 truncate max-w-[140px] block leading-tight">
                  {currentTenant?.brandName || currentTenant?.name || 'Trường Sơn 36'}
                </span>
              </div>
              <span className="sm:hidden font-bold font-mono text-[11px] bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded">
                {currentTenant?.code || 'CT36'}
              </span>
      ...
      <TenantManagerModal
        isOpen={isTenantManagerOpen}
        onClose={() => setIsTenantManagerOpen(false)}
        tenants={tenants}
        activeTenantId={activeTenantId}
        onSelectTenant={handleSelectTenant}
        currentUser={currentUser}
